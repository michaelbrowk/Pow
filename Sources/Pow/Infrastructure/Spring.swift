...
internal struct Spring {
    ...
    func peakTime<V: VectorArithmetic>(initialPosition x0: V, initialVelocity v0: V) -> Double {
        ...
        if zeta < 1 {
            guard v0 != .zero else { return CGFloat.pi / omega1 }
            ...
            return clamp(0, secantMethod(f: derivative, 0, period / (CGFloat.pi * stiffness)), 3)
        } else {
...
internal extension Spring {
    var response: Double {
        (2 * CGFloat.pi) / sqrt(stiffness * mass)
    }
}

private extension Spring {
    var dampingCoefficient: Double {
        4 * CGFloat.pi * zeta * mass / response
    }
